 ...
 *********************/
#define DEF_RADIUS 2
#define DEF_SHADOW_COLOR lv_color_hex3(0x000)
...
static void basic_init(void) {
    ...
    static lv_style_t style_prr;
    lv_style_copy(&style_prr, &lv_style_plain);
    style_prr.body.main_color = REP_PANEL_DARK;
    style_prr.body.grad_color = REP_PANEL_DARK;
    lv_obj_set_style(lv_scr_act(), &style_prr);
}
...
static void btn_init(void) {
    ...
    rel.body.padding.bottom = LV_DPI / 12;
    ...
    rel.text.color = REP_PANEL_DARK_TEXT;
    rel.image.color = lv_color_hsv_to_rgb(_hue, 5, 95);

    lv_style_copy(&pr, &rel);
    pr.body.main_color = REP_PANEL_DARK;
    ...
    ina.text.color = REP_PANEL_DARK_TEXT;
...
static void label_init(void) {
    ...
    sec.text.color = lv_color_hsv_to_rgb(_hue, 80, 210);

    lv_style_copy(&hint, &prim);
    hint.text.color = lv_color_hsv_to_rgb(_hue, 40, 255);
...
static void kb_init(void) {
    ...
    rel.text.font = _font;
    rel.body.border.width = 1;
    rel.body.border.color = REP_PANEL_DARK_ACCENT_ALT2;
    rel.body.border.opa = LV_OPA_30;

    theme.style.kb.bg = theme.style.btnm.bg;
    theme.style.kb.bg->text.color = REP_PANEL_DARK_ACCENT;
